        ...
        try:
            ...
            if page_num == 1:
                ...
                print("Expected number of pages: {}"
                      .format(math.ceil(json_res['count']/page_size)
                              - page_num + 1))
    ...
    if save:
        try:
            with open('../_data/{}_ids.txt'.format(query), 'x') as out_text:
                print("\nSaving to {}".format('../_data/{}_ids.txt'
                                              .format(query)))
...
def save_mp3s(id_list, name='unknown', start_enum=1,
              size_threshold='100MB', overwrite=False):
    # First result has enum value of 1, instead of 0
    enum = start_enum

    print("Query: {}".format(name))
    pathlib.Path('../_data/mp3s/{}'.format(name))\
        .mkdir(parents=True, exist_ok=True)

    for file_id in id_list:
        file_path = "../_data/mp3s/{0}/{2:04}_{0}_{1}.mp3"\
            .format(name, file_id, enum)

        print("======== Result no: {} | File id: {} ========"
              .format(enum, file_id))
        ...
        try:
            with open(file_path, mode) as handle:
                res = requests.get('https://freesound.org/apiv2/sounds/{}/'
                                   .format(file_id),
                                   params={'token': token,
                                           })
                json_res = json.loads(res.content.decode('utf8'))
...
def fetch_mp3s(query, startindex=0, endindex=1,
               size_threshold='100MB', overwrite=False):
    ...
    save_mp3s(query_list, query, start_enum=startindex+1,
